...
import useCustomFunction from "../hooks/useCustomFunction.js";
...
export default function Home() {
  ...
  return (
    <div
      className={`z-10 relative px-4 w-full max-w-xl lg:max-w-[1440px] mx-auto transition-all duration-500 ${s.text}`}
    >
      <Header
      ...
      <div className="w-full pb-16 mt-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-4 xl:col-span-3 flex flex-col gap-6 lg:sticky lg:top-24">
            <div
              className={`${s.card} border rounded-[32px] p-8 shadow-2xl relative overflow-hidden group hidden lg:block`}
            >
              <div
                className={`absolute -right-20 -top-20 w-60 h-60 ${theme === "dark" ? "bg-cyan-500/5" : "bg-blue-500/5"} rounded-full blur-[80px] group-hover:scale-110 transition-all duration-700`}
              ></div>
              <CurrentTime />
            </div>
          ...
          <div className="lg:col-span-8 xl:col-span-9 flex flex-col gap-8">
            <div
              className={`${s.card} border rounded-[32px] p-4 lg:p-8 shadow-xl relative overflow-hidden`}
            >
              <div
                className={`absolute -left-20 -bottom-20 w-60 h-60 ${theme === "dark" ? "bg-cyan-500/5" : "bg-blue-500/5"} rounded-full blur-[80px] pointer-events-none`}
              ></div>
              <SandITime time={time} />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              ...
              >
                ...
                <div className="flex flex-col gap-2 z-10 w-2/3">
                  <h3
                    className={`text-xl font-bold transition-colors duration-300 ${theme === "dark" ? "text-white/90 group-hover:text-amber-300" : "text-slate-800 group-hover:text-emerald-700"}`}
                  >
                    Collections
                  </h3>
                  <p
                    className={`text-sm transition-colors ${theme === "dark" ? "text-emerald-400 group-hover:text-emerald-300" : "text-emerald-600 group-hover:text-emerald-800"}`}
                  >
                    Saved Ayahs & Duas
                  </p>
                  <div
                    className={`w-10 h-1 rounded-full mt-2 group-hover:w-16 transition-all duration-500 ${theme === "dark" ? "bg-emerald-500/50 group-hover:bg-amber-400" : "bg-emerald-200 group-hover:bg-emerald-500"}`}
                  />
                </div>

                <div
                  className={`z-10 p-4 rounded-2xl border transition-all group-hover:-rotate-3 group-hover:scale-110 shadow-lg relative ${theme === "dark" ? "bg-white/5 border-white/10 group-hover:bg-white/10" : "bg-emerald-50 border-emerald-200 group-hover:bg-emerald-100"}`}
                >
