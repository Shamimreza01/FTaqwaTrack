...
export default function Header({
  ...
  return (
    <div className="pt-6 pb-2 flex justify-center w-full relative z-50">
      ...
      >
        <div
          className={`flex items-center justify-center ${loading && !isEditing ? "animate-pulse" : ""}`}
        >
          {isEditing ? (
            <Search
        ...
        {!isEditing && (
          ...
          >
            <MapPin
              className={`w-3.5 h-3.5 ${theme === "dark" ? "text-cyan-400 group-hover:text-emerald-400" : "text-emerald-600"}`}
            />
          </button>
        )}
        ...
        <AnimatePresence>
          {isEditing && (searchQuery.length >= 3 || isSearching) && (
            ...
            >
              {isSearching ? (
                <div
                  className={`p-4 text-center text-sm font-bold flex justify-center items-center gap-2 ${s.textSecondary} opacity-60`}
                >
                ...
                searchResults.map((city, idx) => (
                  ...
                  >
                    ...
                    <div
                      className={`text-[10px] ${s.textSecondary} opacity-60 truncate`}
                    >
                      {city.formatted}
                    </div>
                  </button>
                ...
                <div
                  className={`p-4 text-center text-sm font-bold ${s.textSecondary} opacity-60`}
                >
                  No cities found.
                </div>
              )}
