...
export default function QuranEditionList() {
    ...
    const loadQuran = async () => {
        try {
            ...
            if (storedData) {
                ...
                const responses = await fetch("https://api-taqwatrack.onrender.com/QuranArBnEnAudio").then((res) => res.json());
    ...
    return isLoading ? (
        ...
        <div className="font-[system-ui] py-[80px] px-4 max-w-4xl mx-auto min-h-screen text-white">
            ...
            {filteredSurahs.length === 0 && <div className="text-center text-white/40 mt-10">No surahs found matching "{searchQuery}"</div>}
        </div>
    );
}
