...
export default function DuaItemCard({ dua, dataName, uniqueId, isMemorizeMode, isMemorized, onToggleMemorized }) {
    const [isRevealed, setIsRevealed] = useState(false);
    const { arabicFontSize, translationFontSize, duaBookmarkId, toggleDuaBookmark, favoriteDuaIds, toggleFavoriteDua } = useSettings();
            ...
            <AnimatePresence mode="wait">
                {(!isMemorizeMode || isRevealed) ? (
                    ...
                    >
                        ...
                        >
                            <span className="text-emerald-400 font-bold mr-3">{dua.dua_number || dua.ayah_number}.</span>
                            {dua.bangla_translation}
                            <div className="text-white/60 text-sm mt-3 italic">
                                {dua.bangla_pronunciation}
                            </div>
                        </div>

                        <div className="flex flex-col sm:flex-row items-center gap-4 border-t border-white/5 pt-4 mt-2 justify-between">
                            {dua.audio_link && navigator.onLine ? (
                                <div className="flex-1 w-full max-w-[200px] rounded-xl overflow-hidden shadow-inner bg-white/10" onClick={(e) => e.stopPropagation()}>
                                ...
                                <div className="flex-1">
                                    <span className="text-[10px] uppercase font-bold tracking-widest text-emerald-500/80 bg-emerald-500/10 px-3 py-1 rounded-full border border-emerald-500/20 max-w-[40%] text-right truncate">
                                        {dua.reference}
                                    </span>
                                </div>
                            )}
                            ...
                            <div className="flex items-center gap-3 w-full sm:w-auto shrink-0 justify-end">
                                <button
                                    onClick={(e) => { e.stopPropagation(); toggleFavoriteDua(uniqueId); }}
                                ...
                                <button
                                    onClick={(e) => { e.stopPropagation(); toggleDuaBookmark(uniqueId); }}
                    ...
                    >
                        <div className="absolute right-4 text-[40px] font-arabic opacity-10 blur-md select-none">{dua.arabic.substring(0, 30)}...</div>
                        ...
                        <p className="text-emerald-400/50 font-medium tracking-wide text-sm z-10">Tap to reveal Dua</p>
                    </motion.div>
                )}
            </AnimatePresence>
